...
def attributes_of_sphinx_obj(keyword, sphinx_obj, energy_channel_key, threshold_key):
    ...
    if keyword == "Model": 
        ...
        print(sphinx_obj.prediction.last_data_time_to_issue_time())
...
def attributes_of_mismatch_sphinx_obj(keyword, sphinx_obj, energy_channel_key, threshold_key, mm_pred_energy_channel_key, mm_pred_thresh_key):
    """
    SAME AS THE FUNCTION ABOVE BUT FOR THE MISMATCH UNITTEST SINCE THAT OBJECT IS DIFFERENT
    Function that takes the keyword from the SPHINX dataframe and matches the keyword
    to the matched sphinx object to compare/assertEqual to - ensures that the 
    dataframe is being correctly built
    """
    ...
    if keyword == "Model": 
        ...
        attribute = getattr(sphinx_obj, 'observed_start_time', '')#[threshold_key[energy_channel_key][0]]
    elif keyword == 'Observed SEP End Time':
...
class TestAllClear0(unittest.TestCase):
    ...
    def step_1(self):
        ...
        self.dataframe = validate.fill_df(self.sphinx, self.model_names, self.all_energy_channels, \
            self.obs_thresholds, self.profname_dict, self.DoResume)
